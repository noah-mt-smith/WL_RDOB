...
We can get z-values and p-values for these point estimates and confidence intervals using z-tests. Since the SEs and point estimates are from the GLMM, we can divide the point estimates by the SE to approximate a comparison to 0 (i.e. no change from baseline) based on the model.
